 ...
 *
 *  1. le calculateur y prend `estimerSejour` — le calcul n'existe plus qu'ici,
 *     et il est teste par shared/tarifs.test.js ;
 *  2. le harnais de QA lit les constantes et relit le texte affiche de chaque
 *     page : un montant en euros par kilometre ou un pourcentage d'acompte qui
 *     ne correspond pas est nomme, page et phrase a l'appui.
 *
 ...
 *
 * ── Pourquoi ce fichier n'utilise PAS `export` ──────────────────────────
 *
 * Il a d'abord ete ecrit en module ES, et le calculateur l'importait. C'etait
 * une regression : **un module ES ne se charge pas depuis `file://`**. Ouvrir
 * la page en double-cliquant dessus ne chargeait alors plus aucun script et
 * les boutons ne repondaient plus — sans message visible, sauf a ouvrir la
 * console, ou le navigateur annonce un blocage CORS depuis « origin null ».
 * Signale par Irina le 17 aout 2026, sur la page qui chiffre les gardes.
 *
 * Le fichier pose donc son contenu sur `globalThis.MiaouTarifs`, ce qui le
 * rend utilisable des deux cotes sans rien dupliquer :
 *
 *   <script src="shared/tarifs.js"></script>     script classique : il se
 *                                                charge depuis file:// comme
 *                                                depuis un serveur
 *
 *   import './tarifs.js';                        sous Node, on l'importe pour
 *   const { estimerSejour } = MiaouTarifs;       son effet, puis on lit le
 *                                                global qu'il a pose
 *
 * Ne pas y remettre `export` : cela reintroduirait la panne. Un fichier sans
 * `import` ni `export` reste un module ES valide, donc Node l'accepte tel
 * quel.
 */
...
(function (racine) {
  ...
  function arrondirCentimes(montant) {
   ...
   */
  function estimerSejour({ chats = 1, visites = 1, solidaire = false, periode = false, km = 0 } = {}) {
  ...
  racine.MiaouTarifs = Object.freeze({
    FRAIS_KM_EUR,
    TAUX_ACOMPTE,
    RAYON_INCLUS_KM,
    RAYON_DEVIS_KM,
